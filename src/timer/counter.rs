...
impl Timer<SYSTICK> {
    ...
    pub fn counter_hz0(self) -> SysCounterHz {
        SysCounterHz(self)
    }
...
/// SYSTICKick timer with precision of 1 μs (1 MHz sampling)
pub struct SysCounter<const FREQ: u32>(Timer<SYSTICK>);
...
impl<const FREQ: u32> SysCounter<FREQ> {
    ...
    pub fn now(&self) -> TimerInstantU32<FREQ> {
        TimerInstantU32::from_ticks(SYSTICK::get_current() / (self.clk.raw() / FREQ))
    }
