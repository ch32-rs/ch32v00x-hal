    ...
    ($($TIMX:ident: ($timX:ident),)+) => {
        $(
                ...
                PINS: Pins<REMAP>,
            {
                ...
                // and the used input
                tim.chctlr1_input().modify( |_,w| unsafe {w.cc1s().bits(1).cc2s().bits(1)});
                ...
                PINS: Pins<REMAP>,
            {
                ...
                pub fn read_frequency(&self, mode : ReadMode, clocks : &Clocks) -> Result<Hertz,Error> {
                    ...
                    let ccr1 = unsafe { (*$TIMX::ptr()).ch1cvr.read().bits() as u16};
                ...
                pub fn read_duty(&self, mode : ReadMode) -> Result<(u16,u16),Error> {
                    ...
                    let ccr1 = unsafe { (*$TIMX::ptr()).ch1cvr.read().bits() as u16};
                    let ccr2 = unsafe { (*$TIMX::ptr()).ch2cvr.read().bits() as u16};
                    if ccr1 == 0 {
                        Err(Error::FrequencyTooLow)
                    } else {
                        Ok((ccr2,ccr1))
